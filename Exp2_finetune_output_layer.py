...
x = pretrained.layers[-2].output  # Assumes penultimate layer is Dropout
new_output = layers.Dense(1, activation="sigmoid")(x)
model_exp2 = keras.Model(pretrained.input, new_output)
...
callbacks = [        
    keras.callbacks.ModelCheckpoint(  
        os.path.join(args.log_dir,"best_epx2_model.keras"),  
        save_best_only=True,  
...
txt_path = "Experiment2_results.txt"
...
with open(txt_path, "w") as txt_file, open(csv_path, "w", newline='') as csv_file:
    ...
    txt_file.write("Experiment 2:replace only the output layer of the model:\n")

    for epoch in range(len(history_exp2.history['accuracy'])):
        train_acc = history_exp2.history['accuracy'][epoch]
        val_acc = history_exp2.history['val_accuracy'][epoch]
        train_loss = history_exp2.history['loss'][epoch]
        val_loss = history_exp2.history['val_loss'][epoch]
