...
model_exp3 = keras.Model(pretrained.input, new_output)

# Identify convolutional layers
conv_layers = [layer for layer in model_exp3.layers 
               if isinstance(layer, (layers.Conv2D, layers.SeparableConv2D))]

# Unfreeze first two convolutional layers
for layer in conv_layers[:2]:
    layer.trainable = True
for layer in model_exp3.layers:
    if layer not in conv_layers[:2] and layer != model_exp3.layers[-1]:
...
txt_path = "Experiment3_results.txt"
csv_path = "Experiment3_results.csv"
...
with open(txt_path, "w") as txt_file, open(csv_path, "w", newline='') as csv_file:
    ...
    print("\nExperiment 3: replace the output layer and the first two convolutional layers:")
    txt_file.write("Experiment 3: replace the output layer and the first two convolutional layers:\n")

    for epoch in range(len(history_exp3.history['accuracy'])):
        train_acc = history_exp3.history['accuracy'][epoch]
        val_acc = history_exp3.history['val_accuracy'][epoch]
        train_loss = history_exp3.history['loss'][epoch]
        val_loss = history_exp3.history['val_loss'][epoch]
